...
namespace OrderProcessor.Pipeline.Steps
{
    public class SaveStep: IPipelineStep
    {
        ...
        public async Task<Order> ExecuteAsync(Order input, Func<Order, Task<Order>>? next)
        {
            ...
            {
                input.Status = "Failed";
                _logger.LogError(ex, "ОШИБКА: Не получилось сохранить результаты процесса {ID}", input.ID);
                return input;
            }
            catch(Exception ex)
            {
                input.Status = "Failed";
                _logger.LogError(ex, "ОШИБКА: При сохранении результатов процесса {ID} произошла непредвиденная ошибка: {message}", input.ID, ex.Message);
                return input;
            }
